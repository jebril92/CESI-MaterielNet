...
#pragma endregion
	...
	private: System::Void button7_Click(System::Object^ sender, System::EventArgs^ e) {
			...
			con->Open();
			String^ selected_eid = dataGridView1->CurrentRow->Cells["id_personnel"]->Value->ToString(); // Récupère l'Id de la personnes sur la ligne selectionnée.
			MySqlCommand^ cmd = gcnew MySqlCommand("delete from projet.personnel where id_personnel = '" + selected_eid + "'", con); // Prend l'id correspondant et supprime la ligne selectionnée.
			cmd->ExecuteReader();
			con->Close();
